...
namespace Holiday
{
    public class Class1
    {
        ...
        public Class1()
        {
            ...
            string solutionDir = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location);
            while (!Directory.GetFiles(solutionDir, "*.cs").Any())
            {
            ...
            List<Flight> lstFlight = JsonConvert.DeserializeObject<List<Flight>>(File.ReadAllText(solutionDir + "/flights.json"));
            List<Hotel> lstHotel = JsonConvert.DeserializeObject<List<Hotel>>(File.ReadAllText(solutionDir + "/hotels.json"));
